...
switch (yytype)
    ...
    struct symbol_type : basic_symbol<by_type>
    {
      ...
      {
        YY_ASSERT (tok == token::IDENTIFIER || tok == token::LBRACKET || tok == token::AND || tok == token::RBRACKET || tok == token::LSS || tok == token::TIMES || tok == token::INTEGER_LITERAL || tok == token::PLUS || tok == token::MINUS || tok == token::PERIOD || tok == token::LENGTH || tok == token::TRUE || tok == token::FALSE || tok == token::THIS || tok == token::NEW || tok == token::INT || tok == token::BOOLEAN || tok == token::LPAREN || tok == token::RPAREN || tok == token::EXCLAMATION || tok == token::COMMA || tok == token::LBRACE || tok == token::RBRACE || tok == token::IF || tok == token::ELSE || tok == token::WHILE || tok == token::PRINTLN || tok == token::SEMICOLON || tok == token::EQUAL || tok == token::RETURN || tok == token::PUBLIC || tok == token::CLASS || tok == token::EXTENDS || tok == token::STATIC || tok == token::VOID || tok == token::MAIN || tok == token::STRING || tok == token::WORDSTRING);
      }
#else
    ...
    // Implementation of make_symbol for each symbol type.
      ...
      static
      symbol_type
      make_LSS (std::string v)
      {
        return symbol_type (token::LSS, std::move (v));
      }
#else
      static
      symbol_type
      make_LSS (const std::string& v)
      {
        return symbol_type (token::LSS, v);
      }
#endif
#if 201103L <= YY_CPLUSPLUS
      static
      ...
      static
      symbol_type
      make_WORDSTRING (std::string v)
      {
        return symbol_type (token::WORDSTRING, std::move (v));
      }
#else
      static
      symbol_type
      make_WORDSTRING (const std::string& v)
      {
        return symbol_type (token::WORDSTRING, v);
      }
#endif
